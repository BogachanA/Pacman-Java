 ...
 * dependent on time using reDraw() method. Opening and closing windows in the game are managed by usage of 
...
public class PacComp extends JComponent // by Kaan Cinar
{  
  ...
  public PacComp() //in constructor variables are initialized
  {
    ...
    //variables for the foods
    foods = new Food[20][14];
    //food array is filled with for loops
    for(int i=0;i<19;i++)
      for(int j=0;j<13;j++)
    {
      foods[i][j] = new Food(i*52,j*52);
    }
    //foods in the corners are set as powerup foods
  ...
  public void paintComponent(Graphics g) //draws every object
  {
    ...
    for(Ghost gh:ghosts){//draws ghosts
      ...
      if(gh.getX()<=0) gh.setDirection('E'); //this are used to avoiding the escape of the ghosts through portal
  ...
  public void reDraw() //repaints and sets variables dependent on time, this method is called in timer in the PacFrame
  {
    ...
    if(!dead){
      for(Ghost g:ghosts)
        if(g.boundary.intersects(pac.border)) {
        if(!ghostEat){//if pacman is not in ghost eating phase
          scores.died(); //he dies
          dead = true; 
        }
        else {
          g.ghostEaten(); //ghost get eaten
        }
      }
    }
    ...
    if(dead) counter++; //counter to give pacman some time
    if(counter==50) {dead = false; counter = 0;} //when counter is 50 time is ended, pacman can be eaten again
    ...
    for(int i=0;i<19;i++)
      for(int j=0;j<13;j++){
      if(foods[i][j]!=null) if(pac.getBorderOfPac().intersects(foods[i][j].getBorderOfFood())){ //if intersection happens...
        if(foods[i][j].powerUp) for(Ghost g:ghosts) {g.enterEscapeMode(); ghostEat = true;} //if pacman eats superfood, ghosts run away
        foods[i][j] = null; //as the foods are eaten they are removed
        scores.getPoint(); //as pacman eats food score is increased
      }
    }
